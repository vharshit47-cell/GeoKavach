...
export function FloatingSafetyTools() {
  ...
  const chatContent = <>
<div className="live-chat-header"><Bot size={23} /><div><h2 id="live-chat-title">SurakshaSetu AI Assistant</h2><span>{t("live.derived")}</span></div><button onClick={clear} aria-label={t("live.clearChat")} title={t("live.clearChat")}><Trash2 size={16} /></button>{!chatHost && <button onClick={() => setChatOpen(false)} aria-label={t("live.minimize")}><Minus size={20} /></button>}</div>
