...
import { alertAnchor, caseValue, hazardLabel, numberLabel, relocationLink, riskStyle, RISK_SCALE, type MapHazard, type Selection } from "@/frontend/lib/map/presentation";
  ...
  return <aside className="atlas-insights atlas-panel atlas-glass" aria-label="Risk Insights">
    ...
    <div className="atlas-panel-body">
      ...
      {site ? <>
        ...
        <span className="atlas-badge">OFFICIAL · NDMA SACHET</span><section><Metric label="Severity" value={selection.record.severity} /><Metric label="Hazard" value={selection.record.hazardType} /><Metric label="Affected area" value={selection.record.affectedArea || "See official bulletin"} /><Metric label="Published" value={timeLabel(selection.record.publishedAt, "en")} /><p className="atlas-muted">{selection.record.description}</p>{selection.record.action && <p>{selection.record.action}</p>}<Source url={selection.record.sourceUrl} /></section><p className="atlas-footnote">{alertAnchor(selection.record) ? "Official warning geometry." : "Exact warning boundaries are unavailable. Regional labels are state references, not hazard locations."} This is not a declared relocation red zone; population exposure has not been assessed by this feed.</p>
