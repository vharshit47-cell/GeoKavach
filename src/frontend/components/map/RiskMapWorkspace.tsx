...
import { BarChart3, ChevronRight, LoaderCircle, MapPin, RefreshCw, X } from "lucide-react";
...
import { alertAnchor, alertRegions, mapAlerts, inViewport, type MapHazard, type MapLayers, type MapMode, type Selection, type Viewport } from "@/frontend/lib/map/presentation";
...
export function RiskMapWorkspace() {
  const { location, data, nationalAlerts, nationalEarthquakes, nationalLoading: loading, nationalError: error, selectLocation, selectMapLocation, locateOnce, locating, refresh } = useLocationSafety();
  ...
  const [layers, setLayers] = useState<MapLayers>({ habitations: false, sites: true, redZones: false, alerts: true, earthquakes: true, facilities: false, weather: false, states: true, districts: false });
  ...
  const alertsSource = nationalAlerts;
  const earthquakesSource = nationalEarthquakes;
  const alerts = useMemo(() => mapAlerts(alertsSource?.data ?? [], data?.alerts.data ?? [], hazard), [alertsSource, data?.alerts, hazard]);
  ...
  const focus = useMemo(() => {
    ...
    if (selection?.kind === "alert") { const point = alertAnchor(selection.record); const region = alertRegions(selection.record)[0]; return point ? { ...point, zoom: 9 } : region ? { ...region, zoom: 6 } : null; }
  ...
  const select = useCallback((next: Selection) => { setSelection(next); setSummaryOpen(false); setInsightsOpen(true); setDrawer("insights"); }, []);
  const chooseLocation = useCallback((point: LocationPoint) => { setSelection(null); setInsightsOpen(false); setDrawer(null); if (point.state || point.name) selectLocation(point); else selectMapLocation(point); }, [selectLocation, selectMapLocation]);
  ...
  const noGeometry = alerts.filter(alert => !alertAnchor(alert)).length;
  ...
  return <AppShell title="Hazard Intelligence" description="Explore hazards, assess exposure and connect evidence to relocation decisions." actions={<button className="atlas-page-refresh" onClick={refreshAll} disabled={loading || workflow.loading}><RefreshCw size={13} />Refresh evidence</button>}>
    <section className={`risk-atlas atlas-simple ${insightsOpen ? "has-insights" : ""} ${drawer ? `atlas-drawer-${drawer}` : ""}`} aria-label="Suraksha Setu hazard intelligence map">
      ...
      {!summaryOpen && <button className="atlas-restore-summary atlas-glass" onClick={() => { setSummaryOpen(true); setInsightsOpen(false); setDrawer("summary"); }}><span className="atlas-warning-count">{loading && !alertsSource ? "…" : alertsSource && alertsSource.status !== "unavailable" ? alerts.length : "—"}</span><span>Official warnings<small>India · selected hazard</small></span><ChevronRight size={15} /></button>}
    ...
    <div className="atlas-data-footer"><span>NDMA SACHET · USGS · Open-Meteo · OpenStreetMap</span><span>{noGeometry > 0 ? `${noGeometry} warnings have no precise geometry. Regional warning labels refer to named states, not hazard locations or extents. ` : ""}Warning polygons are not declared red zones. Boundaries: {states.result?.data?.license || "reference GIS"}.</span><details><summary>Location tools & source notes</summary><p>Saved field records retain the existing risk and capacity rules. Historical hazard layers and declared red-zone polygons are not connected. No disaster observations or assessments are generated for display.</p><GoogleLocationMap location={location} /></details></div>
  </AppShell>;
}
